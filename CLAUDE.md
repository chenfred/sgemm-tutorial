...
这是一个**学习 CUDA core 编程**的练习仓库，以单精度矩阵乘（SGEMM）为载体，通过实现多个版本来对比和分析性能。当前保留 `sgemm_v0` 与 `sgemm_v1` 两版。改动以「新增一个优化版本 + 对比性能」为主要工作模式。

## 关键约定：标准的 M/N/K 语义

本仓库的 GEMM 定义是 **C(M,N) = A(M,K) × B(K,N)**，其中 **K 是收缩（reduce/求和）维度**（cuBLAS 标准约定）。

- `A` 布局 M×K，`B` 布局 K×N，`C` 布局 M×N（见 `src/main.cpp` 的 `bytes_*`、初始化范围与 `src/verify.cpp` 的 CPU 参考实现）。
...
3. 在 `src/main.cpp` 的 `IMPLEMENTATIONS` 中注册 `{"sgemm_vN", sgemm_vN_do}`。
4. `scripts/build.sh --run` 编译并跑对比；`scripts/profile.sh -o sgemm_vN` 生成 ncu 报告。

`include/sgemm_func.h` 用统一函数指针类型约束所有 kernel 的签名；`main.cpp` 负责 GPU 执行和计时，`verify.cpp` 负责多线程 CPU golden 与正确性校验。
...
scripts/build.sh --run --dry-run # 编译后只跑数据生成/H2D/kernel/D2H
scripts/profile.sh -o sgemm_vN   # Nsight Compute 全量报告 → ncu-rep/sgemm_vN.ncu-rep
```

也可直接 `cmake -S . -B build && cmake --build build`。脚本均通过 `SCRIPT_DIR`/`PROJECT_ROOT` 实现任意目录可执行。

**没有独立的 test 框架**——`main.cpp` 调用 `sgemm_golden` 与 `sgemm_verify`，按 FP32 混合容差逐元素比较。为简化当前 v0/v1 报告，默认只跑 `1024×4096×1024`；新增版本时还应临时加入非方阵且不能被 tile 整除的尺寸检验边界。
...
- `ncu-rep/`（ncu 报告）与 `build/` 已在 `.gitignore`。

## 编码风格（沿用现有代码）

4 空格缩进、不用 tab；所有 `for`、`if` 等控制流都使用花括号；kernel 用小写下划线（`sgemm_v0`），host 封装加 `_do` 后缀（`sgemm_v0_do`）；宏全大写（`CUDA_CHECK`）；tile size 等用 `constexpr`（`TILE_SIZE`/`TILE_LEN`）。改 kernel 优先保证边界正确，再谈性能。
