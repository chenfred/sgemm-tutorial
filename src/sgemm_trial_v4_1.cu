...
// v4_1 仅将 v3 的 LDG -> prefetch registers -> STS 替换为
// cp.async，保留双缓冲。 标准 SGEMM 语义：C(M,N) = A(M,K) * B(K,N)，K
// 为收缩维度。
__global__ void sgemm_trial_v4_1(const float* A, const float* B, float* C, int M, int N, int K) {
...
void sgemm_trial_v4_1_do(const float* A, const float* B, float* C, int M, int N, int K) {
    dim3 blockDim{TILEBASE_X, TILEBASE_Y};
    dim3 gridDim{CeilDiv<u32>(N, TILE_X), CeilDiv<u32>(M, TILE_Y)};

    CUDA_CHECK(
        cudaFuncSetAttribute(sgemm_trial_v4_1, cudaFuncAttributeMaxDynamicSharedMemorySize, sizeof(SharedLayout)));

    sgemm_trial_v4_1<<<gridDim, blockDim, sizeof(SharedLayout), nullptr>>>(A, B, C, M, N, K);
}
